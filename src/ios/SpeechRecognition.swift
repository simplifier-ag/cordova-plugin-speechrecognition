...
@objc (SpeechRecognition) class SpeechRecognition : CDVPlugin {
    ...
    func startListening(command: CDVInvokedUrlCommand){
        ...
        let language: String = command.argument(at: 0, withDefault: SpeechRecognition.DEFAULT_LANGUAGE) as? String
            ?? SpeechRecognition.DEFAULT_LANGUAGE
        ...
        askMicPermission { (granted, message) in
            DispatchQueue.main.async {
                ...
                if !(self.speechRecognizer?.isAvailable ?? false) {
                    ...
                    let pluginResult = CDVPluginResult(status: CDVCommandStatus_ERROR, messageAs: SpeechRecognition.MESSAGE_NOT_DETERMINED)
                    self.commandDelegate.send(pluginResult, callbackId: command.callbackId)
                    return
                }
                ...
                self.recognitionRequest?.shouldReportPartialResults = true//showPartial
    ...
    private func askMicPermission(completion: @escaping (Bool, String) -> ()) {
        SFSpeechRecognizer.requestAuthorization { status in
            ...
            switch status {
            case .denied:
                message = SpeechRecognition.MESSAGE_ACCESS_DENIED
                break
            case .restricted:
                message = SpeechRecognition.MESSAGE_RESTRICTED
                break
            case .notDetermined:
                message = SpeechRecognition.MESSAGE_NOT_DETERMINED
                break
            case .authorized:
                message = ""
                break
            ...
            completion(status == .authorized, message)
        }
    }
